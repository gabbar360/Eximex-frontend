          ...
          <div className="space-y-2">
            {duePayments.slice(0, 3).map((payment) => (
              <div key={payment.id} className="text-sm text-red-700">
                {payment.piInvoice?.piNumber} - {payment.party?.companyName} - ₹{payment.dueAmount}
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">PI Total</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Advance Received</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {payments && payments.length > 0 ? payments.map((payment) => (
                <tr key={payment.id}>
                  <td className="px-6 py-4 text-sm font-medium">{payment.piInvoice?.piNumber}</td>
                  <td className="px-6 py-4 text-sm">{payment.party?.companyName}</td>
                  <td className="px-6 py-4 text-sm font-semibold">₹{payment.amount}</td>
                  <td className="px-6 py-4 text-sm text-green-600 font-medium">₹{payment.piInvoice?.advanceAmount || 0}</td>
                  <td className="px-6 py-4 text-sm text-red-600 font-medium">₹{payment.amount - (payment.piInvoice?.advanceAmount || 0)}</td>
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-gray-500">
                    No payments found. Create a PI Invoice to generate payment records.
                  </td>
                </tr>
              )}
            </tbody>
