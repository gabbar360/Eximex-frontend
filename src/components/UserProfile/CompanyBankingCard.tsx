...
interface CompanyBankingCardProps {
  companyData: CompanyData | null;
  onUpdate: (data: Partial<CompanyData>) => Promise<void>;
}

export default function CompanyBankingCard({
  companyData,
  onUpdate,
}: CompanyBankingCardProps) {
  ...
  const handleSave = async () => {
    try {
      console.log('Sending banking data:', formData);
      await onUpdate(formData);
      toast.success('Banking details updated successfully!');
      closeModal();
    } catch (error) {
      console.log('Update error:', error);
      toast.error(error?.message || 'Failed to update banking details');
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update your company banking information.
            </p>
          </div>
          <form
