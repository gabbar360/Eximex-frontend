...
const SimpleOrderCard: React.FC<SimpleOrderCardProps> = ({ order, onDelete }) => {
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      pending: { color: 'bg-yellow-100 text-yellow-800 border-yellow-200', label: 'Pending' },
      confirmed: { color: 'bg-blue-100 text-blue-800 border-blue-200', label: 'Confirmed' },
      processing: { color: 'bg-purple-100 text-purple-800 border-purple-200', label: 'In-Production' },
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300 p-4 mb-4">
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg flex-shrink-0">
            <FontAwesomeIcon icon={faFileAlt} className="text-blue-600 dark:text-blue-400" />
          </div>
        ...
        <div className="text-right">
          ...
          <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${orderStatus.color} mt-1`}>
            {orderStatus.label}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        ...
        >
          <FontAwesomeIcon icon={faShip} className="text-blue-600 dark:text-blue-400 text-sm" />
          <span className="text-sm text-blue-700 dark:text-blue-300 font-medium">Shipment</span>
        </Link>
        ...
        >
          <FontAwesomeIcon icon={faList} className="text-green-600 dark:text-green-400 text-sm" />
          <span className="text-sm text-green-700 dark:text-green-300 font-medium">Packing</span>
        </Link>
        ...
        >
          <FontAwesomeIcon icon={faBalanceScale} className="text-purple-600 dark:text-purple-400 text-sm" />
          <span className="text-sm text-purple-700 dark:text-purple-300 font-medium">VGM</span>
        </Link>
        ...
        >
          <FontAwesomeIcon icon={faDownload} className="text-orange-600 dark:text-orange-400 text-sm" />
          <span className="text-sm text-orange-700 dark:text-orange-300 font-medium">Reports</span>
        </Link>
      </div>
