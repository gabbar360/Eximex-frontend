    ...
    <div className="p-2 sm:p-4 lg:p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-2 sm:gap-3 lg:gap-4 xl:gap-6 mb-4 sm:mb-6 lg:mb-8">
          ...
          <MetricCard title="Products" value={stats.totalProducts} icon={<MdInventory className="w-4 h-4" />} />
          <MetricCard title="Orders" value={stats.totalOrders} icon={<MdShoppingCart className="w-4 h-4" />} />
          <MetricCard title="Shipments" value={stats.totalShipments} icon={<MdShoppingCart className="w-4 h-4" />} />
          <MetricCard title="Packing Lists" value={stats.totalPackingLists} icon={<MdInventory className="w-4 h-4" />} />

          <MetricCard title="VGM Documents" value={stats.totalVgmDocuments} icon={<MdAttachMoney className="w-4 h-4" />} />
          <MetricCard title="Purchase Orders" value={stats.totalPurchaseOrders} icon={<MdShoppingCart className="w-4 h-4" />} />
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chart1Data} margin={{ top: 10, right: 10, left: 5, bottom: 10 }}>
                ...
                <YAxis domain={[0, (dataMax) => Math.ceil(dataMax + dataMax * 0.1)]} allowDecimals={false} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#6b7280' }} />
                <Tooltip contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '11px' }} />
                <Legend wrapperStyle={{ fontSize: '11px' }} />

                <Bar dataKey="contacts" fill="url(#contactsGradient)" name="Contacts" radius={[2, 2, 0, 0]} maxBarSize={30} />
                <Bar dataKey="products" fill="url(#productsGradient)" name="Products" radius={[2, 2, 0, 0]} maxBarSize={30} />
                <Line dataKey="piInvoices" stroke="#10b981" strokeWidth={2} name="PI Invoices" type="monotone" dot={{ r: 3 }} />
                <Line dataKey="orders" stroke="#3b82f6" strokeWidth={2} name="Orders" type="monotone" dot={{ r: 3 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </ChartCard>
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chart2Data} margin={{ top: 10, right: 10, left: 5, bottom: 10 }}>
                ...
                <Bar dataKey="packing" fill="url(#packingGradient)" name="Packing Lists" radius={[2, 2, 0, 0]} maxBarSize={30} />
                <Bar dataKey="purchaseOrders" fill="url(#purchaseGradient)" name="Purchase Orders" radius={[2, 2, 0, 0]} maxBarSize={30} />
                <Bar dataKey="shipments" fill="url(#shipmentsGradient)" name="Shipments" radius={[2, 2, 0, 0]} maxBarSize={30} />
                <Bar dataKey="vgm" fill="url(#vgmGradient)" name="VGM Documents" radius={[2, 2, 0, 0]} maxBarSize={30} />
